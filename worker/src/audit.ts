...
<head>
  ...
  <style>
    ...
    *{box-sizing:border-box}body{margin:0}.shell{width:min(1080px,calc(100% - 32px));margin:42px auto}
    ...
    .toolbar{display:flex;justify-content:space-between;gap:10px;padding:14px;margin-bottom:16px}.toolbar .group{display:flex;gap:8px}
...
<body><main class="shell">
  ...
  <section class="toolbar"><span class="muted">管理员会话已通过 Authenticator 验证</span><div class="group"><a href="/" class="muted">凭据中心</a><button id="load">刷新队列</button></div></section>
  <div id="status"></div><section id="list" class="list"></section>
</main>
<script>
const list=document.querySelector('#list'),statusBox=document.querySelector('#status');
document.querySelector('#load').addEventListener('click',load);
async function api(path,options={}){const response=await fetch(path,{credentials:'same-origin',...options,headers:{'content-type':'application/json',...(options.headers||{})}});const data=await response.json();if(!response.ok)throw new Error(data.error||('HTTP '+response.status));return data}
